...
const metadataTemplate = document.getElementById('metadataItemTemplate');
...
const viewTemplate = document.getElementById('viewItemTemplate');
...
function renderMetadataList(filter = '') {
    ...
    const matches = METADATA.filter((field) => {
        if (!normalized)
            return true;
        return (field.label.toLowerCase().includes(normalized) ||
    ...
    matches.forEach((field) => {
        const fragment = metadataTemplate.content.cloneNode(true);
...
function createView(name, metadataIds) {
    return {
...
function showViewDetails(viewId) {
